...
int main(){
	...
	vector<pair<const Person, int>> mVec;
	srand(time(NULL));
	Person p1("a");
	Person p2("c");
	Person p3("e");
	...
	mVec.push_back(make_pair(p5, 5));
	mVec.push_back(make_pair(p6, 6));
	mVec.push_back(make_pair(p7, 7));
	mVec.push_back(make_pair(p8, 8));

	m.insert(mVec.begin(), mVec.end());
	for(auto it = m.begin(); it != m.end(); ++it){
		cout << (*it).first.name << endl;
	}

	cs540::Map<const Person, int> mCopy(m);
	cout << (mCopy < m) << endl;
